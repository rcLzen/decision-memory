...
function buildPage(decision) {
    ...
    for (const line of lines) {
        if (line.startsWith('DECISION:'))
            decisionLine = line.replace('DECISION:', '').trim();
        else if (line.startsWith('RATIONALE:'))
            rationaleLine = line.replace('RATIONALE:', '').trim();
        else if (line.startsWith('IMPLICATION:'))
            implicationLine = line.replace('IMPLICATION:', '').trim();
    }
    // Fallback: use the whole text if not structured
    if (!decisionLine)
        decisionLine = decision.subject;
...
function detectPhase(text) {
    ...
    if (t.includes('triage') || t.includes('ai-triage'))
        return 'phase-1-ai-triage';
    if (t.includes('review') || t.includes('workbench') || t.includes('layout'))
        return 'phase-2-review-workbench';
    if (t.includes('e2e') || t.includes('test') || t.includes('playwright'))
        return 'phase-3-e2e';
    if (t.includes('db') || t.includes('database') || t.includes('reset'))
...
//# sourceMappingURL=write-to-gbrain.js.map